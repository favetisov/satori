...
function translateSVGNodeToSVGString(
    ...
    if (restProps.style && restProps.style['opacity'] !== undefined) {
        restProps.style = 'opacity: ' + restProps.style.opacity;
    } else {
...
export function SVGNodeToSVG(node: ReactElement): string {
    let {
        viewBox,
        viewbox,
        width,
    ...
    restProps.width = width || viewBoxSize[2];
    restProps.height = height || viewBoxSize[3];
    if (style && style['opacity'] !== undefined) {
        restProps.style = 'opacity: ' + style.opacity;
    }

    return `<svg${Object.entries(restProps)
        .map(([k, _v]) => {
            return ` ${ATTRIBUTE_MAPPING[k] || k}="${_v}"`
        })
        .join('')}>${translateSVGNodeToSVGString(children)}</svg>`;
