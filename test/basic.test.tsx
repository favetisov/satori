...
describe('Basic', () => {
  ...
  it('should render empty div', async () => {
    const data = JSON.parse(readFileSync(__dirname + '/data.json').toString());
    const svg = await satori(data, { width: 200, height: 200, fonts });
    // const data = <div style={{height: '100px', width: '100px', backgroundColor: 'darkred', clipPath: 'polygon(20% 0%, 80% 0%, 100% 100%, 0% 100%)'}}/>
    // const svg = await satori(data, { width: 100, height: 100, fonts });

    writeFileSync(__dirname + '/test.svg', svg);
